 ...
 Color GetSpaceColor(Color baseColor, Vector3 velocity, Vector3 position,
     ...
     float gravFactor = (distance > rs * 1.01f)
         ? sqrtf(fmaxf(0.0f, 1.0f - rs / distance)) : 0.0f;
 ...
 void DrawScene(const std::vector<Photon>& photons, const blackHole& bh, Camera3D& camera) {
     ...
     for (const auto& photon : photons) {
         ...
         Color col = GetSpaceColor(photon.baseColor, photon.velocity,
                                   photon.position, camera.position, dist, rs);
 
         
         if (photon.historyCount > 1) {
             for (int i = 0; i < photon.historyCount - 1; i++) {
                
                 int ci = (photon.historyIndex - photon.historyCount + i     + MAX_HISTORY) % MAX_HISTORY;
                 int ni = (photon.historyIndex - photon.historyCount + i + 1 + MAX_HISTORY) % MAX_HISTORY;
 
                 
                 float t = (float)i / (float)(photon.historyCount - 1);
 
                 
                 float alpha = t * t;
 
                 DrawLine3D(photon.history[ci], photon.history[ni], Fade(col, alpha));
             }
         }
 
         
         if (photon.body) {
             DrawSphere(photon.position, 0.5f, col);
         }
     }
     EndBlendMode();
